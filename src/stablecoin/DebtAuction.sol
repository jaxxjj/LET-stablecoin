...
contract DebtAuction is Auth, CircuitBreaker {
    ...
    uint48 public auction_duration = 2 days; // 2 days total auction length if no one bids need to restart
    // kicks - Total auction count, used to track auction ids
    uint256 public last_auction_id = 0;
    // vow
    address public ds_engine; // not used until shutdown
    ...
    // participants make a bid, decreasing the lot size (Submit a fixed LET bid with decreasing lot size)
    function bid(uint256 id, uint256 lot, uint256 bid_amount) external not_stopped {
        ...
        require(block.timestamp < b.bid_expiry_time || b.bid_expiry_time == 0, "already finished bid");
        ...
        if (msg.sender != b.highest_bidder) {
            ...
            if (b.bid_expiry_time == 0) {
                uint256 debt = IDSEngine(b.highest_bidder).total_debt_on_debt_auction();
                // On first dent, bid_expiry_time = 0 and highest_bidder = vow
                IDSEngine(b.highest_bidder).decrease_auction_debt(Math.min(bid_amount, debt));
            }
