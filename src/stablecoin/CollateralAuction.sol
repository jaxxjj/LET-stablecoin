...
contract CollateralAuction is Auth, Guard {
    ...
    // vow - Recipient of LET raised in auctions
    ...
    // Emitted when new auction is started
    // id: Unique auction identifier
    // starting_price: Initial collateral price
    // coin_amount: Target stablecoin amount to raise
    // collateral_amount: Amount of collateral for sale
    // user: Original CDP owner
    // keeper: Address receiving incentive fees
    // fee: Total incentive fee paid
    event Start(
        uint256 indexed id,
    ...
    // Emitted when collateral is purchased from auction
    // id: Auction identifier
    // max_collateral: Maximum collateral buyer was willing to purchase
    // price: Actual price paid per unit
    // owe: Total amount of stablecoin paid
    // coin_amount: Remaining stablecoin to be raised
    // collateral_amount: Remaining collateral for sale
    // user: Buyer address
    event Take(
        uint256 indexed id,
    ...
    // Emitted when auction is reset due to timeout or price drop
    // Parameters match Start event
    event Redo(
        uint256 indexed id,
        uint256 starting_price,
        uint256 coin_amount,
    ...
    // Emitted when auction is cancelled via emergency stop
    event Yank(uint256 id);

    // --- Init ---
    constructor(address _cdp_engine, address _spotter, address _liquidation_engine, bytes32 _collateral_type) {
    ...
    function redo(
        ...
        uint256 fee;
        if (flat_fee > 0 || fee_rate > 0) {
            if (coin_amount >= min_coin && collateral_amount * price >= min_coin) {
                fee = flat_fee + Math.wmul(coin_amount, fee_rate);
                cdp_engine.mint({debt_dst: ds_engine, coin_dst: keeper, rad: fee});
            }
        }
    ...
    //
    // Auctions will not collect more LET than their assigned LET target,`coin_amount`;
    // thus, if `amt` would cost more LET than `coin_amount` at the current price, the
    // amount of collateral purchased will instead be just enough to collect `coin_amount` LET.
    //
    // To avoid partial purchases resulting in very small leftover auctions that will
    // never be cleared, any partial purchase must leave at least `Clipper.chost`
    // remaining LET target. `chost` is an asynchronously updated value equal to
    ...
    function take(
        ...
        uint256 max_collateral,
        // max [ray] - Maximum acceptable price (LET / collateral)
        uint256 max_price,
        ...
        uint256 collateral_amount = sale.collateral_amount;
        uint256 coin_amount = sale.coin_amount;
        // LET needed to buy a slice of this sale
        uint256 owe;
        {
            ...
            // Removes LET out for liquidation from accumulator
    ...
    function update_min_coin() external {
        ICDPEngine.Collateral memory col = ICDPEngine(cdp_engine).collaterals(collateral_type);
        min_coin = Math.wmul(col.min_debt, liquidation_engine.penalty(collateral_type));
    }
    ...
    function yank(uint256 id) external auth lock {
        ...
        cdp_engine.transfer_collateral(collateral_type, address(this), msg.sender, sale.collateral_amount);
        _remove(id);
        emit Yank(id);
    }
}
