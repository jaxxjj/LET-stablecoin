...
// DssCdpManager
contract CDPManager {
    event OpenCDP(address indexed user, address indexed owner, uint256 indexed cdp_id);
    ...
    mapping(address => mapping(uint256 => mapping(address => bool))) public cdp_can;
    ...
    modifier cdp_allowed(uint256 cdp_id) {
        ...
        require(msg.sender == owner || cdp_can[owner][cdp_id][msg.sender], "cdp not allowed");
        _;
    }

    // urnAllowed
    modifier cdp_handler_allowed(address user) {
        require(msg.sender == user || cdp_handler_can[user][msg.sender], "cdp handler not allowed");
        _;
    }
    ...
    function allow_cdp(uint256 cdp_id, address user, bool ok) public cdp_allowed(cdp_id) {
    ...
    function modify_cdp(uint256 cdp_id, int256 delta_col, int256 delta_debt) public cdp_allowed(cdp_id) {
    ...
    function transfer_collateral(uint256 cdp_id, address dst, uint256 wad) public cdp_allowed(cdp_id) {
        ICDPEngine(cdp_engine).transfer_collateral(collaterals[cdp_id], positions[cdp_id], dst, wad);
    }
    ...
    function transfer_collateral(bytes32 col_type, uint256 cdp_id, address dst, uint256 wad)
        public
        cdp_allowed(cdp_id)
    {
        ICDPEngine(cdp_engine).transfer_collateral(col_type, positions[cdp_id], dst, wad);
    }

    // move
    // Transfer wad amount of BEI from the cdp_id address to a dst address.
    function transfer_coin(uint256 cdp_id, address dst, uint256 rad) public cdp_allowed(cdp_id) {
    ...
    function quit(uint256 cdp_id, address cdp_dst) public cdp_allowed(cdp_id) cdp_handler_allowed(cdp_dst) {
        bytes32 col_type = collaterals[cdp_id];
        address cdp = positions[cdp_id];

        ICDPEngine.Position memory pos = ICDPEngine(cdp_engine).positions(col_type, cdp);
    ...
    function enter(address cdp_src, uint256 cdp_id) public cdp_handler_allowed(cdp_src) cdp_allowed(cdp_id) {
        bytes32 col_type = collaterals[cdp_id];

        ICDPEngine.Position memory pos = ICDPEngine(cdp_engine).positions(col_type, cdp_src);
    ...
    function shift(uint256 cdp_src, uint256 cdp_dst) public cdp_allowed(cdp_src) cdp_allowed(cdp_dst) {
        require(collaterals[cdp_src] == collaterals[cdp_dst], "not matching collaterals");
        ICDPEngine.Position memory pos = ICDPEngine(cdp_engine).positions(collaterals[cdp_src], positions[cdp_src]);
        ICDPEngine(cdp_engine).fork({
